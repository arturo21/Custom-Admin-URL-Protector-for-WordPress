...
class Custom_Admin_URL_Core {
	...
	public function __construct() {
		...
		$this->secret_slug = $this->get_secret_slug();

		if ( empty( $this->secret_slug ) ) {
			return;
		}

		// Interceptar peticiones en la inicialización temprana de WordPress.
		add_action( 'init', array( $this, 'handle_login_request' ), 1 );

		// Filtrar funciones nativas de generación de URLs en sitio único y Multisite.
	...
	/**
	 * Obtener el slug según si se ejecuta en Multisite o sitio independiente.
	 *
	 * @return string
	 */
	private function get_secret_slug() {
		if ( is_multisite() ) {
			return get_site_option( CUSTOM_ADMIN_URL_OPTION, 'mi-acceso-secreto' );
		}
		return get_option( CUSTOM_ADMIN_URL_OPTION, 'mi-acceso-secreto' );
	}
	...
	public function handle_login_request() {
		...
		if ( $this->is_default_login_path( $parsed_path ) ) {
			...
			// Registrar el intento bloqueado en los logs del servidor.
			$this->log_unauthorized_access( $request_uri );

			// Bloquear acceso devolviendo estado HTTP 404.
	...
	/**
	 * Comprueba si la solicitud es una excepción legítima (AJAX, CRON, XMLRPC, usuarios autenticados o endpoints exentos).
	 *
	...
	private function is_allowed_request( $parsed_path ) {
		...
		// Permitir endpoints indispensables dentro de /wp-admin/ para solicitudes de usuarios no autenticados.
	...
	/**
	 * Registrar en los logs de seguridad los intentos no autorizados.
	 *
